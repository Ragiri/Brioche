...
t_bunny_response loop_function(void *d) {
    ...
    move(&data->dir[0], &data->position[0], data->pix);
    move(&data->dir[1], &data->position[1], data->pix);
    return (GO_ON);
}
...
t_bunny_response display_function(void *d) {
    ...
    std_set_line(data->pix, data->position[0], data->position[1]);
    bunny_blit(&data->win->buffer, &data->pix->clipable, &(t_bunny_position){0, 0});
